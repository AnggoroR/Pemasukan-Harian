...
<body>
    ...
    <section class="container-xxl pb-3">
            ...
            <aside class="rounded-3 shadow-rounded col-lg-12">
                <div class="flex-shrink-0 pt-2 pb-1 fs-5 rounded-top">
                    <a href="#" class="d-block link-primary text-decoration-none text-dark py-1 px-2" title="Profil">
                        <svg xmlns="http://www.w3.org/2000/svg" width="33" class="bi bi-person-circle my-1 mx-2" viewBox="0 0 16 16">
                            <path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0z"/>
                            <path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1z"/>
                        </svg> {{ auth()->user()->name }}
                ...
                <ul class="sidebar-menu list-unstyled container lh-lg">
                    ...
                    <li>
                        <a href="#" class="fs-6 text-decoration-none li-sidebar rounded-pill p-2" style="color: black;">
        ...
        <h2 class="fw-bold mb-4 pt-2">Edit Pendapatan</h2>
        <form action="{{ route("updatePendapatan",$pdp->id) }}" method="POST" class="form-custom px-4 pb-5 pt-4 mb-4">
        @csrf
        <div class="row mb-3">
            <label for="inputEmail3" class="col-sm-2 col-form-label">Uang Masuk</label>
            <div class="col-sm-10">
                <input type="text"  class="form-control @error('uang_masuk') is-invalid @enderror" id="inputEmail3" placeholder="Input uang" name="uang_masuk" value="{{ $pdp->uang_masuk }}">
                @error('uang_masuk')
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-10">
                <input type="date" class="form-control" id="inputEmail3" name="tanggal_masuk" value="{{ $pdp->tanggal_masuk }}">
            </div>
        </div>
        <div class="row mb-3">
            {{-- @php
            ...
            <div class="col-sm-10">
                <input type="text" class="form-control @error('kategori_pendapatan') is-invalid @enderror" id="inputEmail3" placeholder="Berikan keterangan" name="kategori_pendapatan" required value="{{ $pdp->kategori_pendapatan }}">
                    @error('kategori_pendapatan')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                    @enderror
                {{-- <select name="kategori_pendapatan" id="kategori_pendapatan"  class="form-control">
                    @foreach ($semua_kategori_pdp as $kategori_pendapatan)
                        @if ($kategori_pendapatan == $pdp->kategori_pendapatan)
                        <option value="{{ $kategori_pendapatan }}" selected>{{ $kategori_pendapatan }}</option>
                        @else
                        <option value="{{ $kategori_pendapatan }}">{{ $kategori_pendapatan }}</option>
                        @endif
                    @endforeach
                </select> --}}
            </div>
